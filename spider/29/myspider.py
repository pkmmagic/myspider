...
def part_3_go(masterPath, downloadLogPath, candidatesPath):
    since = time.time()
    try:
        master_list = []
        ...
        print(e)
    n = master2candidates(master_list, candidatesPath)
    print('%s jpgs download success from master list' % str(n))
    time_elapsed = round((time.time() - since), 2)
    print(time_elapsed)
    try:
        log(downloadLogPath, str(len(master_list)), time_elapsed)
        print('logging to %s success' % downloadLogPath)
    except Exception as e:
        print(e)
